...
@RestController
@RequestMapping("/api")
@CrossOrigin(origins = "*") // 모든 도메인에서 요청 가능하도록 CORS 설정
public class SogdamController {
    
    private final String[] sogdams = {
    ...
    @GetMapping("/sogdam")
    public ResponseEntity<?> getRandomSogdam() {
        Random random = new Random();
        String selectedSogdam = sogdams[random.nextInt(sogdams.length)];
        
        return ResponseEntity.ok()
                .header("Content-Type", "application/json; charset=UTF-8")
                .body(new ApiResponse(200, selectedSogdam));
    }
